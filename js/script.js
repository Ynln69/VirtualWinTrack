document.addEventListener("DOMContentLoaded", () => {
  ...
  if (currentPath === "home.html" || currentPath === "") {
    loadHomePageContent() // New function for dynamic sections
    loadHotelCardsHome()
...
function displayErrorMessage(containerElementOrSelector, message) {
  ...
  if (container) {
    ...
    console.error("Error message container not found:", containerElementOrSelector)
  }
}
...
async function loadHomePageContent() {
  ...
  if (aboutSection && content.aboutUs) {
    const paragraphsHtml = content.aboutUs.paragraphs.map((p) => `<p>${p}</p>`).join("")
...
async function loadHotelCardsHome() {
  ...
  if (hotels && hotels.length > 0) {
    ...
    hotels.forEach((hotel) => {
      ...
      card.className = "hotel-card-home" // Use home-specific class
      card.innerHTML = `
          <img src="${hotel.photo || "/placeholder.svg?width=400&height=300"}" alt="${hotel.name}">
    ...
    displayErrorMessage(container, "Could not load hotels. Please try again later.")
  }
}
...
async function loadHotelDetailsPage() {
  ...
  if (hotel) {
    showcaseContainer.innerHTML = `
      <h1>${hotel.name}</h1>
      <img src="${hotel.photo || "/placeholder.svg?width=1200&height=500"}" alt="${hotel.name}" class="hotel-main-image-details">
    `
    descriptionMain.innerHTML = `<h2>About ${hotel.name}</h2><p>${hotel.fullDescription || hotel.description}</p>`

    if (hotel.gallery && hotel.gallery.length > 0) {
      galleryAside.innerHTML = `<h3>Photo Gallery</h3><div class="gallery-thumbnails">
        ${hotel.gallery.map((imgUrl) => `<img src="${imgUrl}" alt="Gallery image for ${hotel.name}" onclick="updateMainImage('${imgUrl}', '${hotel.name}')">`).join("")}
      </div>`
    } else {
      galleryAside.innerHTML = "<h3>Photo Gallery</h3><p>No additional photos available.</p>"
    }
  } else {
  ...
  if (hotelEvents && hotelEvents.length > 0) {
    ...
    hotelEvents.forEach((event) => {
      ...
      eventItem.className = "event-item-details" // Use details-specific class
      eventItem.setAttribute("data-event-id", event.id)
      eventItem.innerHTML = `
          <h4>${event.name}</h4>
          <p class="event-date-details">Date: ${new Date(event.date).toLocaleDateString()}</p>
    ...
    eventsListContainer.innerHTML = "<p>No events scheduled for this hotel at the moment.</p>"
  }
}
...
async function loadPromotionsIdeasPage() {
  ...
  if (promotions && promotions.length > 0) {
    ...
    promotions.forEach((promo) => {
      const promoCard = document.createElement("article") // Use article for semantic meaning
      promoCard.className = "idea-card" // Use ideas-specific class
      promoCard.innerHTML = `
          <div class="idea-card-image-wrapper">
              <img src="${promo.image || "/placeholder.svg?width=600&height=400"}" alt="${promo.title || "Promotion"}" class="idea-card-image">
          </div>
          <div class="idea-card-content">
              <h3>${promo.title || "Special Offer"}</h3>
              <p>${promo.description}</p>
              <div class="idea-card-footer">
                  ${promo.hotelLogo ? `<img src="${promo.hotelLogo || "/placeholder.svg?width=100&height=50"}" alt="Hotel Logo" class="idea-card-logo">` : "<div></div>"}
                  ${promo.link ? `<a href="${promo.link}" target="_blank" rel="noopener noreferrer" class="idea-card-link">Learn More</a>` : ""}
              </div>
          </div>
      `
    ...
    displayErrorMessage(container, "Could not load holiday ideas. Please try again later.")
  }
}
